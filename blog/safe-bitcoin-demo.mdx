...
This is an example of how a virtual chain could be a runtime environment for another blockchain lacking native smart contracts. [The idea](/crosschain/chain-signatures) could be applied in general to any virtual chain and any accounts or contracts on it.
<!-- truncate -->

## Acknowledgements

I want to say big "Thank You" to people helping me out while creating this demo:
- Michael Birch for [XCC] consultations.
- Oleksandr Anyshchenko for writing the main part of the [Chain Signatures Signer] contract and support with demo.
...
The first one explaining the theory behind the demo and overall idea:
...
The second one demonstrating how everything could be executed on practice:
...
Some blockchains are lacking smart contracts: Bitcoin, Ripple, etc.. 
Or have non EVM architectures, and no Solidity contracts available. 
But you are an EVM dev, who wants to build there. Also, you want to have a scalabitity of the NEAR Protocol at the tips of your hands.

Then [NEAR Chain Signatures] + a virtual chain, like Aurora, is a way to resolve this problem for you! 

I am gonna demonstrate you how it could be done by creating a Safe wallet on Aurora and controlling the derived Bitcoin addresses by it.
...
To do this two pieces of technology is combined:
1. [XCC].
...
This gives us an implementation of Chain Signatures on Aurora via Solidity contract called [Chain Signatures Signer].

You can read more about it in the documentation about [Chain Signatures on Virtual Chains] and the repo's `README.md` itself.

I believe that picture can convey the whole process in a best possible way, so here is it:
...
So, we have our Safe wallet with two signers – Safe User 1 and Safe User 2 assigned to it. We assume Safe User 1 is doing most of the actions, and Safe User 2 just need to confirm the transaction.
...
Here is a list of docs and resources which explain all parts needed to understand the demo perfectly:
...
- [Chain Signatures JS Lib] - some tools to derive accounts and relay the transaction take from [NEAR Multichain Examples]
...
2. To put the signed data back to your console.

Then, a transaction will be relayed and executed on Bitcoin network.
